...
export async function activate(context: vscode.ExtensionContext) {
    ...
    let baseExt = vscode.extensions.getExtension("undefined_publisher.ai4sd-helpers");
    if (!baseExt) {
        await vscode.commands.executeCommand("ai4sd-helpers.wakeup");
        baseExt = vscode.extensions.getExtension("undefined_publisher.ai4sd-helpers");
        if (!baseExt) {
            vscode.window.showErrorMessage("ai4sd-helpers extension is not installed");
            return;
        }
    }
    baseExt.exports.register({label: "EasyComment", description: "Executes EasyComment", execute: () => vscode.commands.executeCommand("EasyComment.annotate")});
