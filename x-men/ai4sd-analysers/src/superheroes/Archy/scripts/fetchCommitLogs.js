...
export async function fetchCommitLogsFromWorkspace(commitLogsFileWorkspacePath) {
    const workspacePath = vscode.workspace.workspaceFolders?.[0].uri.fsPath;
    const commitLogsFileName = path.basename(commitLogsFileWorkspacePath);
    const git = simpleGit(workspacePath); // git variable acts as interface to the git repository, instead of running shell commands
    try {
        ...
        vscode.window.showErrorMessage(`Error fetching commit logs: ${error.message}`);
    }
}
