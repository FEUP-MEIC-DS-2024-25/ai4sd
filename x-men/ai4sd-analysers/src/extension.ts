...
export function activate(context: vscode.ExtensionContext) {
	...
	const disposable = vscode.commands.registerCommand('ai4sd-analysers.call', async () => {
		...
		const options: vscode.QuickPickItem[] = [
			...
			{ label: "ADR Generator", description: "Executes ADR Generator superhero" },
		  ...
		  if (selectedOption) {
			switch (selectedOption.label) {
			  ...
			  case "SARA":
				...
				break;
			  case "ADR Generator":
			    // Import and execute ADR Generator
				(await import('./superheroes/adr_generator/adr_generator.js')).execute(context);
				break;
			  case "Template":
