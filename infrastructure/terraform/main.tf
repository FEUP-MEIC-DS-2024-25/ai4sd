...
###########################
# Grant publishing and subscribing permissions for echo-jarvis topic
resource "google_pubsub_topic_iam_member" "jarvis_pubsub" {
  topic  = google_pubsub_topic.echo_jarvis.name
  member = "serviceAccount:${google_service_account.jarvis.email}"
  role   = "roles/pubsub.editor"  # Publisher and subscriber
}

resource "google_pubsub_topic_iam_member" "superheroes_pubsub_echo_jarvis" {
  for_each = toset(local.superhero_names)
  topic    = google_pubsub_topic.echo_jarvis.name
  member   = "serviceAccount:${google_service_account.superhero[each.key].email}"
  role     = "roles/pubsub.editor"  # Publisher and subscriber
...
resource "google_pubsub_topic_iam_member" "strange_pubsub_echo_jarvis" {
  topic  = google_pubsub_topic.echo_jarvis.name
  member = "serviceAccount:${google_service_account.strange.email}"
  role   = "roles/pubsub.editor"  # Publisher and subscriber
}

# Grant publishing and subscribing permissions for echo-superheroes topic
resource "google_pubsub_topic_iam_member" "superheroes_pubsub_echo_superheroes" {
  for_each = toset(local.superhero_names)
  topic    = google_pubsub_topic.echo_superheroes.name
  member   = "serviceAccount:${google_service_account.superhero[each.key].email}"
  role     = "roles/pubsub.editor"  # Publisher and subscriber
...
resource "google_pubsub_topic_iam_member" "strange_pubsub_echo_superheroes" {
  topic  = google_pubsub_topic.echo_superheroes.name
  member = "serviceAccount:${google_service_account.strange.email}"
  role   = "roles/pubsub.editor"  # Publisher and subscriber
}
...
resource "google_project_iam_member" "superheroes_firestore" {
  project = "your-project-id"
  role    = "roles/datastore.admin"
  member  = "serviceAccount:superhero-0X-0Y@your-project-id.iam.gserviceaccount.com"
}

resource "google_project_iam_member" "jarvis_firestore" {
  project = "your-project-id"
  role    = "roles/datastore.admin"
  member  = "serviceAccount:echo-jarvis@your-project-id.iam.gserviceaccount.com"
}

resource "google_project_iam_member" "strange_firestore" {
  project = "your-project-id"
  role    = "roles/datastore.admin"
  member  = "serviceAccount:echo-strange@your-project-id.iam.gserviceaccount.com"
}

############################


resource "google_storage_bucket_iam_member" "superheroes_nexus" {
  bucket = "nexus"
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:superhero-0X-0Y@your-project-id.iam.gserviceaccount.com"
}

resource "google_storage_bucket_iam_member" "jarvis_nexus" {
  bucket = "nexus"
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:echo-jarvis@your-project-id.iam.gserviceaccount.com"
}

resource "google_storage_bucket_iam_member" "strange_nexus" {
  bucket = "nexus"
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:echo-strange@your-project-id.iam.gserviceaccount.com"
}
