...
export const optionalChainingMutator = {
    ...
    *mutate(path) {
        if (path.isOptionalMemberExpression() && path.node.optional) {
            yield t.optionalMemberExpression(t.cloneNode(path.node.object, true), t.cloneNode(path.node.property, true), path.node.computed, 
            /*optional*/ false);
        }
        if (path.isOptionalCallExpression() && path.node.optional) {
            yield t.optionalCallExpression(t.cloneNode(path.node.callee, true), path.node.arguments.map((arg) => t.cloneNode(arg, true)), 
            /*optional*/ false);
        }
    },
};
