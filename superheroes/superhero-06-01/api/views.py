...
from api.utils.github_api import get_issues, get_user_stories, get_commits, get_contributors_activity, get_all_commits
...
def analyze_repo_commits(request, repo_owner, repo_name):
    ...
    prompt = (
        ...
        "            \"links\": \"\" // string (links to the evidence: commit codes)\n"
        "          }\n"
        "        ]\n"
        "      }\n"
        "    ],\n"
...
def analyze_repo_issues(request, repo_owner, repo_name):
    ...
    prompt = (
        ...
        "            \"path\": \"\", // string (file path or branch name)\n"
        "            \"reason\": \"\" // string (explanation of why this is evidence)\n"
        "            \"links\": \"\" // string (links to the evidence: issue codes)\n"
        "          }\n"
        "        ]\n"
        "      }\n"
        "    ],\n"
...
def analyze_user_stories(request, repo_owner, repo_name):
    ...
    prompt = (
        ...
        "            \"path\": \"\", // string (file path or branch name)\n"
        "            \"reason\": \"\" // string (explanation of why this is evidence)\n"
        "            \"links\": \"\" // string (links to the evidence: user story codes)\n"
        "          }\n"
        "        ]\n"
        "      }\n"
        "    ],\n"
...
def analyze_contributors_activity(request, repo_owner, repo_name):
    ...
    if not contributors_activity:
        return Response({"error": "Failed to fetch contributors' activity."}, status=404)
...
def analyze_commit_sizes(request, repo_owner, repo_name):
    ...
    for commit in commits:
        ...
        commit_size = commit.get("stats", {}).get("total", 0)  # Requires enabling commit stats API
...
def analyze_full_repo(request, repo_owner, repo_name):
    ...
    if not (commits or issues or user_stories or contributors_activity):
        return Response({"error": "Failed to fetch any data from the repository."}, status=404)
    ...
    prompt = (
        ...
        "            \"path\": \"\", // string (file path or branch name)\n"
        "            \"reason\": \"\" // string (explanation of why this is evidence)\n"
        "            \"links\": \"\" // string (links to the evidence: commit codes, issues and user story codes...)\n"
        "          }\n"
        "        ]\n"
        "      }\n"
        "    ],\n"
